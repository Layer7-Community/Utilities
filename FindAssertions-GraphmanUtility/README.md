...
- **Graphman Configuration**: The `graphman.configuration` file `options` section supports two values for `policyCodeFormat`:
  ...
  Both formats are supported and detected automatically. Example configurations:
  ```json
  { "options": { "policyCodeFormat": "Code" } }
  ```
  ```json
  { "options": { "policyCodeFormat": "json" } }
  ```
...
2. Searches for the specified assertion in both services and policies:
   - Automatically handles both Graphman `policyCodeFormat` settings:
     - **`"Code"`** format: reads policy from `policy.code` (parsed JSON object)
     - **`"json"`** format: reads policy from `policy.json` (JSON string, parsed at runtime)
   - **Services**: Deep-searches the resolved policy code object for the assertion
   - **Policies**: Deep-searches the resolved policy code object for the assertion
...
- **Dual policy format support**: Automatically detects and handles both `policyCodeFormat` settings — `"Code"` (parsed object under `policy.code`) and `"json"` (JSON string under `policy.json`)
- **Deep recursive search**: Traverses the full policy code structure to locate the assertion regardless of nesting depth
...
- **Dual format handling**: Automatically resolves policy code from `policy.code` (object) or `policy.json` (string), handling JSON parse errors for the latter gracefully
- **Missing properties**: Handles missing or null policy structures gracefully
...
- **v1.7**: Latest updates
  ...
  - No Graphman configuration change is required; the script detects the format at runtime
  - Updated prerequisites documentation to reflect that both formats are supported

- **v1.6**: Previous updates
